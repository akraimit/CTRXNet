...
if output_video_path == "":
	...
	output_video_path = input_video_path.split('.')[0] + "_CRTXNet.mp4"
...
# In order to draw the trajectory of ball, we need to save the coordinate of preious 7 frames 
q = collections.deque()
for i in range(0,8):
	q.appendleft(None)
...
while(True):
	...
	if not ret or currentFrame == 20: 
		break

	#img is the frame that CRTXNet will predict the position
	...
	#since CRTXNet output is ( net_output_height*model_output_width , n_classes )
	...
	#check if there have any ball be detected
	if circles is not None:
		#if only one ball be detected
		if len(circles) == 1:
	...
	#if circles is not None:
        #        cv2.imshow("Ball Frame", opencvImage)
